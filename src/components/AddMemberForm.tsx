...
const AddMemberForm = ({ onSubmit }: AddMemberFormProps) => {
  ...
  useEffect(() => {
    // استرجاع أنواع العضويات المحفوظة
    const storedTypes = localStorage.getItem('membershipTypes')
    if (storedTypes) {
      try {
        const parsedTypes = JSON.parse(storedTypes)
        setMembershipTypes(Object.values(parsedTypes))
      } catch (error) {
        console.error('خطأ في قراءة أنواع العضويات', error)
      }
    }
  }, [])
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    try {
      if (!formData.image) {
        throw new Error('الرجاء اختيار صورة للعضو')
      }
